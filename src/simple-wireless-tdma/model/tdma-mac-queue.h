...
public:
  ...
  void SetNetDevicePtr (Ptr<TdmaNetDevice> netDevicePtr);
  /**
  ...
  void UpdateNumberActiveFlows();

  void PrintStats();
  
  std::vector<TopkQuery> flows;
  // vector of booleans that relate to active queries. set to true if packet was rcvd in last second
  std::vector<bool> still_active;
...
private:
  ...
  Ptr<TdmaNetDevice> m_netDevicePtr;
