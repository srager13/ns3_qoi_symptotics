...
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac Constructor\n";
  }
  ...
  Simulator::ScheduleNow( &TdmaCentralMac::DoStart, this );
}
TdmaCentralMac::~TdmaCentralMac ()
{
}
...
TdmaCentralMac::NotifyTxDrop (Ptr<const Packet> packet)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::NotifyTxDrop()\n";
  }
...
TdmaCentralMac::SetChannel (Ptr<SimpleWirelessChannel> channel)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::SetChannel()\n";
  }
  if (channel != 0)
    {
      m_channel = channel;
...
TdmaCentralMac::SetTdmaController (Ptr<TdmaController> controller)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::SetTdmaController()\n";
  }
...
TdmaCentralMac::SetDevice (Ptr<TdmaNetDevice> device)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::SetDevice()\n";
  }
  m_device = device;
...
TdmaCentralMac::GetQueueState (uint32_t index)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::GetQueueState()\n";
  }
...
TdmaCentralMac::ForwardUp (Ptr<Packet> packet, Mac48Address from, Mac48Address to)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::ForwardUp()\n";
  }
...
TdmaCentralMac::Queue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  if (!m_queue->Enqueue (packet, hdr))
  {
    if( TDMA_CENTRAL_MAC_DEBUG )
    {
        std::cout <<"\tin TdmaCentralMac::Queue() - dropped packet, should notify via callback\n";
    }
...
TdmaCentralMac::StartTransmission (uint64_t transmissionTimeUs)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::StartTransmission()\n";
  }
  ...
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"Packet TransmissionTime(microSeconds): " << packetTransmissionTime.GetMicroSeconds () << "usec";
  }
...
TdmaCentralMac::SendPacketDown (Time remainingTime)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::SendPacketDown()\n";
  }
  WifiMacHeader header;
...
TdmaCentralMac::Enqueue (Ptr<const Packet> packet, Mac48Address to, Mac48Address from)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::Enqueue()\n";
  }
...
TdmaCentralMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::Enqueue( packet, addressTo) - to " << to << "\n";
  }
...
TdmaCentralMac::TxFailed (const WifiMacHeader &hdr)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::TxFailed()\n";
  }
}
...
TdmaCentralMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::Receive()\n";
  }
...
TdmaCentralMac::DoStart (void)
{
  if( TDMA_CENTRAL_MAC_DEBUG )
  {
    std::cout<<"In TdmaCentralMac::DoStart()...good because we start the tdma-controller here.\n";
  }
  ...
  m_queue->SetNetDevicePtr (m_low->GetDevice());
