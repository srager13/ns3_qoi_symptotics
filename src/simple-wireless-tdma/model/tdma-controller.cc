                                    ...
                                    m_channel (0)
{
  ...
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController constructor\n";
  }
}
...
TdmaController::Start (void)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::Start\n";
  }
  NS_LOG_FUNCTION (this);
  if (!m_activeEpoch)
    {
...
TdmaController::StartTdmaSessions (void)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::StartTdmaSessions\n";
  }
...
TdmaController::AddTdmaSlot (uint32_t slotPos, Ptr<TdmaMac> macPtr)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::AddTdmaSlot\n";
  }
  NS_LOG_FUNCTION (slotPos << macPtr);
  //std::pair<std::map<uint32_t, Ptr<TdmaMac> >::iterator, bool> result =
    m_slotPtrs.push_back (std::make_pair (slotPos,macPtr));
      if( TDMA_CONTROLLER_DEBUG )
      {
        std::cout<<"Added mac : " << macPtr << " in slot " << slotPos << "\n";
      }
  /*if (result.second == true)
  ...
  else
    {
      if( TDMA_CONTROLLER_DEBUG )
      {
        std::cout<<"Could not add mac: " << macPtr << " to slot " << slotPos << "\n";
      }
...
TdmaController::SetSlotTime (Time slotTime)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::SetSlotTime\n";
  }
...
TdmaController::GetSlotTime (void) const
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::GetSlotTime\n";
  }
...
TdmaController::SetDataRate (DataRate bps)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::SetDataRate\n";
  }
...
TdmaController::GetDataRate (void) const
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::GetDataRate\n";
  }
  return m_bps;
}
...
TdmaController::SetChannel (Ptr<SimpleWirelessChannel> c)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::SetChannel()\n";
  }
...
TdmaController::ScheduleTdmaSession (const uint32_t slotNum)
{
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<<"In TdmaController::ScheduleTdmaSession at time " << Simulator::Now().GetSeconds() << " (slotNum = " << slotNum << ")\n";
  }
  NS_LOG_FUNCTION (slotNum);
  //std::vector<uint32_t, Ptr<TdmaMac> >::iterator it = m_slotPtrs.find (slotNum);

  // for every mac assigned to this slot, determine the number of slots (should always be one) and
  //  and then schedule a transmission for that mac
  uint32_t numOfSlotsAllotted = 1;
  Time totalTransmissionTimeUs = Seconds(0);
  for ( uint32_t i = 0; i < m_slotPtrs.size(); i++ )
  {
    if ( m_slotPtrs[i].first == slotNum )
    {
      /*
      while (1)
        {
        ...
        if( TDMA_CONTROLLER_DEBUG )
        {
          std::cout<<"Number of slots allotted for this node is: " << numOfSlotsAllotted << "\n";
        }
      ...
      */
      ...
      totalTransmissionTimeUs = GetGaurdTime () + transmissionSlot;
      //NS_ASSERT (it->second != NULL);
      
      //std::cout<< "\tMac " << m_slotPtrs[i].second << " get to StartTransmission.\n";
      m_slotPtrs[i].second->StartTransmission (transmissionSlot.GetMicroSeconds ());
    }
  }
  if( TDMA_CONTROLLER_DEBUG )
  {
    std::cout<< "\tTotalSlotsAllowed = " << GetTotalSlotsAllowed() << "\n";
  }
  if ((slotNum + numOfSlotsAllotted) == GetTotalSlotsAllowed ())
  {
    if( TDMA_CONTROLLER_DEBUG )
    {
      std::cout<<"\tStarting over all sessions again\n";
    }
  ...
  else
  {
    if( TDMA_CONTROLLER_DEBUG )
    {
      std::cout<< "\tScheduling next session\n";
    }
