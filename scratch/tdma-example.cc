                      ...
                      uint32_t slotTime, uint32_t gaurdTime, uint32_t interFrameGap)
{
  ...
  else
  {
	...
	//oss << "Dsdv_Manet_" << t_nodes << "Nodes_" << macProtocol << "_" << t_txpDistance  "txDistance_" << sTotalTime  "SimTime";
  //std::string tr_name = oss.str();     
	std::string tr_name = "Tdma_Manet.tr";
  std::cout << "Trace file generated is " << tr_name << "\n";
  ...
  if (usingWifi)
    {
      ...
      wifiPhy.EnableAsciiAll (ascii.CreateFileStream (tr_name));
      ...
      for (uint32_t j = 0; j <= m_nSinks - 1; j++ )
        {
          OnOffHelper onoff1 ("ns3::UdpSocketFactory", Address (InetSocketAddress (interfaces.GetAddress (j), port)));
  				onoff1.SetAttribute ("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));// time for one packet to arrive
